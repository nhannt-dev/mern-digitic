...
const InputField = ({ value, setValue, nameKey, type, invalid, setInvalid }) => {
  return (
    <div className='w-full flex flex-col relative mb-2'>
      ...
      <input type={type || 'text'} className='px-4 py-2 rounded-sm border w-full mt-2 placeholder:text-sm placeholder:italic outline-none' placeholder={nameKey.slice(0, 1).toUpperCase() + nameKey.slice(1)} value={value} onChange={e => setValue(prev => ({ ...prev, [nameKey]: e.target.value }))} onFocus={() => setInvalid([])} />
